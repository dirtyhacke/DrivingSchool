...
const Payments = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const studentRes = await fetch('https://drivingschool-9b6b.onrender.com/api/payments/students');
      const sData = await studentRes.json();
      if(sData.success) setStudents(sData.users || []);

      const payRes = await fetch('https://drivingschool-9b6b.onrender.com/api/payments/all');
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await fetch('https://drivingschool-9b6b.onrender.com/api/payments/update', {
        method: 'POST',
        body: data
      });
