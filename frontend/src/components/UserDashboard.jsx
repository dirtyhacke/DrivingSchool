...
import MyAllData from './MyAllData';

const UserDashboard = ({ user, onLogout }) => {
  const [activeTab, setActiveTab] = useState('slots'); // Default to slots
  const [darkMode, setDarkMode] = useState(false); // Changed to false for white mode by default
    ...
    <div className={`fixed inset-0 z-[400] flex flex-col lg:flex-row font-sans transition-colors duration-500 ${darkMode ? 'bg-[#020617] text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <aside className={`hidden lg:flex w-72 border-r flex-col p-6 transition-colors ${darkMode ? 'border-white/5 bg-white/[0.02]' : 'border-slate-200 bg-white'}`}>
        <div className="flex items-center gap-3 mb-10 px-2">
          ...
          <div>
            ...
            <p className="text-[8px] text-blue-600 font-black uppercase tracking-widest mt-1">Driving School</p>
          </div>
        </div>

        <nav className="flex-1 space-y-2">
          {/* Removed Training and Payments */}
        ...
        <div className="pt-4 border-t border-slate-200 space-y-2">
           ...
           <div className={`flex items-center gap-3 p-4 rounded-2xl mb-2 ${darkMode ? 'bg-white/5' : 'bg-slate-100'}`}>
              <div className="w-8 h-8 rounded-lg bg-blue-600/20 flex items-center justify-center text-blue-600 border border-blue-500/20"><User size={16} /></div>
           ...
           <button onClick={onLogout} className="w-full flex items-center gap-4 p-4 text-red-600 font-black text-[10px] uppercase tracking-widest hover:bg-red-500/10 rounded-2xl transition-all">
      ...
      <main className="flex-1 overflow-y-auto pb-20 lg:pb-0">
        ...
        <div className="p-6 lg:p-10 max-w-5xl mx-auto">
          {/* Only show Today Slots, My All Data, and Settings */}
      ...
      <nav className={`lg:hidden fixed bottom-0 left-0 right-0 h-16 border-t flex items-center justify-around px-4 z-50 backdrop-blur-xl ${darkMode ? 'bg-[#020617]/90 border-white/5' : 'bg-white/90 border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]'}`}>
        <MobileBtn icon={<LayoutGrid size={22}/>} active={activeTab === 'slots'} onClick={() => setActiveTab('slots')} darkMode={darkMode} />
        <MobileBtn icon={<Database size={22}/>} active={activeTab === 'alldata'} onClick={() => setActiveTab('alldata')} darkMode={darkMode} />
        <MobileBtn icon={<Settings size={22}/>} active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} darkMode={darkMode} />
        <button onClick={onLogout} className="p-3 text-red-600/70 hover:text-red-600">
          <LogOut size={22}/>
        </button>
      </nav>
    </div>
...
const SidebarBtn = ({icon, label, active, onClick, darkMode}) => (
  <button onClick={onClick} className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all group ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : `opacity-60 hover:opacity-100 ${darkMode ? 'hover:bg-white/5' : 'hover:bg-slate-100'}`}`}>
...
const MobileBtn = ({icon, active, onClick, darkMode}) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all ${active ? 'bg-blue-600 text-white shadow-md' : `text-gray-600 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}`}>
    {icon}
  </button>
);
